...
def cmd_sync(args):
    ...
    # 4. Write app-layer .env files (e.g. app/bifrost/* → /opt/secrets/bifrost.env)
    if bsm_secrets:
        _write_app_envs(bsm_secrets)
...
def _write_app_envs(secrets: list):
    """Generate /opt/secrets/<app>.env for secrets prefixed with app/<app>/."""
    app_envs = {}
    for s in secrets:
        if s.key.startswith("app/"):
            parts = s.key.split("/", 2)
            if len(parts) >= 3:
                app_name = parts[1]
                env_key = parts[2]
                app_envs.setdefault(app_name, {})[env_key] = s.value
    ...
    for app_name, env_vars in app_envs.items():
        ...
        print(f"  📄 Создан {app_file} ({len(env_vars)} ключей)")
